...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    std::string bspline_topic = "/drone_" + std::to_string(planner_manager_->pp_.drone_id) + "_planning/bspline";
    bspline_pub_ = nh.advertise<traj_utils::Bspline>(bspline_topic, 10);
    ROS_INFO("[EGO] bspline will publish to: %s", bspline_topic.c_str());
    //====================================================================
  ...
  {
    ROS_WARN("[EGO] Received traj_start_trigger!");
    ROS_INFO("[EGO DEBUG] odom_ready: %d", have_odom_);
    ROS_INFO("[EGO DEBUG] current height: %f", odom_pos_(2));
    have_trigger_ = true;
    ...
    // 把收到的目标点直接喂给 planNextWaypoint（和 waypointCallback 一致）
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case WAIT_TARGET:
    {
      ...
      else
      {
        ...
        // else
        // {
        changeFSMExecState(GEN_NEW_TRAJ, "FSM"); //原changeFSMExecState(SEQUENTIAL_START, "FSM");
        // }
      }
      break;
    }
      ...
      if (planner_manager_->pp_.drone_id <= 0 || (planner_manager_->pp_.drone_id >= 1 && have_recv_pre_agent_))
      {
        if (have_odom_ && have_target_ && have_trigger_)
        {
          ...
          else
          {
            ...
            changeFSMExecState(SEQUENTIAL_START, "FSM");  //原changeFSMExecState(SEQUENTIAL_START, "FSM");
          }
        }
        else
        {
