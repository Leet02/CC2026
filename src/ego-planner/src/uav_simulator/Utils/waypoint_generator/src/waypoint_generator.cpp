...
ros::Publisher pub2;
ros::Publisher pub3;
ros::Publisher pub_trigger;  // 新添加：发布/traj_start_trigger
...
void goal_callback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
    ...
    trigged_time = ros::Time::now(); // 使用当前时间
    ...
    if (waypoint_type == string("circle")) {
        ...
        if (msg->pose.position.z > -0.1) {
            ...
            // 发布/traj_start_trigger，使用pt作为goal trigger
            pt.header.frame_id = "lio_world";
            pt.header.stamp = ros::Time::now();  // 确保有效stamp
            pub_trigger.publish(pt);
            ROS_INFO("[waypoint_generator] Published trigger to /traj_start_trigger for ego-planner.");
        } else {
...
void traj_start_trigger_callback(const geometry_msgs::PoseStamped& msg) {
    ...
    ROS_INFO_STREAM("Pattern " << waypoint_type << " generated!");  // 改为INFO，避免[ERROR]误导
    ...
    if (waypoint_type == string("free")) {
        ...
        load_waypoints(n, trigged_time);
    } // 移除manual-lonely-waypoint处理，避免循环
}
...
int main(int argc, char** argv) {
    ...
    pub_trigger = n.advertise<geometry_msgs::PoseStamped>("traj_start_trigger", 1);  // 新添加
